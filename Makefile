...
job.name=PageRank.PageRankMain
...
aws.bucket.name= spark-pr-bucket
aws.input=input
aws.output=output
aws.log.dir=log
aws.num.nodes=5
...
aws: jar upload-app-aws delete-output-aws
	aws emr create-cluster \
		...
		--steps Type=CUSTOM_JAR,Name="${app.name}",Jar="command-runner.jar",ActionOnFailure=TERMINATE_CLUSTER,Args=["spark-submit","--deploy-mode","cluster","--class","${job.name}","s3://${aws.bucket.name}/${jar.name}","${aws.k}","s3://${aws.bucket.name}/${aws.input}","s3://${aws.bucket.name}/${aws.output}"] \
